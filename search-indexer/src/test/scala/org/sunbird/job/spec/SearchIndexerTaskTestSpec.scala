...
//  }
  ...
  "Composite Search Indexer" should " sync the Data Node " in {
    when(mockKafkaUtil.kafkaJobRequestSource[Event](jobConfig.kafkaInputTopic)).asInstanceOf[OngoingStubbing[CompositeSearchEventSource]].thenReturn(new CompositeSearchEventSource(List[String](EventFixture.DATA_NODE_CREATE)))
    when(mockKafkaUtil.kafkaStringSink(jobConfig.kafkaErrorTopic)).asInstanceOf[OngoingStubbing[CompositeSearchFailedEventSink]].thenReturn(new CompositeSearchFailedEventSink)
  ...
  "Composite Search Indexer" should " update the Data Node " in {
    when(mockKafkaUtil.kafkaJobRequestSource[Event](jobConfig.kafkaInputTopic)).asInstanceOf[OngoingStubbing[CompositeSearchEventSource]].thenReturn(new CompositeSearchEventSource(List[String](EventFixture.DATA_NODE_UPDATE)))
    when(mockKafkaUtil.kafkaStringSink(jobConfig.kafkaErrorTopic)).asInstanceOf[OngoingStubbing[CompositeSearchFailedEventSink]].thenReturn(new CompositeSearchFailedEventSink)
    new SearchIndexerStreamTask(jobConfig, mockKafkaUtil).process()
  ...
  "Composite Search Indexer" should " create and delete the Data Node " in {
    ...
    when(mockKafkaUtil.kafkaJobRequestSource[Event](jobConfig.kafkaInputTopic)).asInstanceOf[OngoingStubbing[CompositeSearchEventSource]].thenReturn(new CompositeSearchEventSource(List[String](EventFixture.DATA_NODE_CREATE, EventFixture.DATA_NODE_DELETE)))
    when(mockKafkaUtil.kafkaStringSink(jobConfig.kafkaErrorTopic)).asInstanceOf[OngoingStubbing[CompositeSearchFailedEventSink]].thenReturn(new CompositeSearchFailedEventSink)
    new SearchIndexerStreamTask(jobConfig, mockKafkaUtil).process()
  ...
  "Composite Search Indexer" should " do nothing for the Data Node due to UNKNOWN Operation " in {
    embeddedElastic.deleteIndices()
    when(mockKafkaUtil.kafkaJobRequestSource[Event](jobConfig.kafkaInputTopic)).asInstanceOf[OngoingStubbing[CompositeSearchEventSource]].thenReturn(new CompositeSearchEventSource(List[String](EventFixture.DATA_NODE_UNKNOWN)))
    when(mockKafkaUtil.kafkaStringSink(jobConfig.kafkaErrorTopic)).asInstanceOf[OngoingStubbing[CompositeSearchFailedEventSink]].thenReturn(new CompositeSearchFailedEventSink)
    new SearchIndexerStreamTask(jobConfig, mockKafkaUtil).process()
  ...
  "Composite Search Indexer" should " sync the External Dialcode Data " in {
    when(mockKafkaUtil.kafkaJobRequestSource[Event](jobConfig.kafkaInputTopic)).asInstanceOf[OngoingStubbing[CompositeSearchEventSource]].thenReturn(new CompositeSearchEventSource(List[String](EventFixture.DIALCODE_EXTERNAL_CREATE)))
    when(mockKafkaUtil.kafkaStringSink(jobConfig.kafkaErrorTopic)).asInstanceOf[OngoingStubbing[CompositeSearchFailedEventSink]].thenReturn(new CompositeSearchFailedEventSink)
    new SearchIndexerStreamTask(jobConfig, mockKafkaUtil).process()
  ...
  "Composite Search Indexer" should " update the External Dialcode Data " in {
    when(mockKafkaUtil.kafkaJobRequestSource[Event](jobConfig.kafkaInputTopic)).asInstanceOf[OngoingStubbing[CompositeSearchEventSource]].thenReturn(new CompositeSearchEventSource(List[String](EventFixture.DIALCODE_EXTERNAL_UPDATE)))
    when(mockKafkaUtil.kafkaStringSink(jobConfig.kafkaErrorTopic)).asInstanceOf[OngoingStubbing[CompositeSearchFailedEventSink]].thenReturn(new CompositeSearchFailedEventSink)
    new SearchIndexerStreamTask(jobConfig, mockKafkaUtil).process()
  ...
  "Composite Search Indexer" should " create and delete the External Dialcode Data " in {
    embeddedElastic.deleteIndices()
    when(mockKafkaUtil.kafkaJobRequestSource[Event](jobConfig.kafkaInputTopic)).asInstanceOf[OngoingStubbing[CompositeSearchEventSource]].thenReturn(new CompositeSearchEventSource(List[String](EventFixture.DIALCODE_EXTERNAL_CREATE, EventFixture.DIALCODE_EXTERNAL_DELETE)))
    when(mockKafkaUtil.kafkaStringSink(jobConfig.kafkaErrorTopic)).asInstanceOf[OngoingStubbing[CompositeSearchFailedEventSink]].thenReturn(new CompositeSearchFailedEventSink)
    new SearchIndexerStreamTask(jobConfig, mockKafkaUtil).process()
  ...
  "Composite Search Indexer" should " do nothing for the External Dialcode Data due to UNKNOWN Operation " in {
    embeddedElastic.deleteIndices()
    when(mockKafkaUtil.kafkaJobRequestSource[Event](jobConfig.kafkaInputTopic)).asInstanceOf[OngoingStubbing[CompositeSearchEventSource]].thenReturn(new CompositeSearchEventSource(List[String](EventFixture.DIALCODE_EXTERNAL_UNKNOWN)))
    when(mockKafkaUtil.kafkaStringSink(jobConfig.kafkaErrorTopic)).asInstanceOf[OngoingStubbing[CompositeSearchFailedEventSink]].thenReturn(new CompositeSearchFailedEventSink)
    new SearchIndexerStreamTask(jobConfig, mockKafkaUtil).process()
  ...
  "Composite Search Indexer" should " sync the Dialcode Metrics Data " in {
    when(mockKafkaUtil.kafkaJobRequestSource[Event](jobConfig.kafkaInputTopic)).asInstanceOf[OngoingStubbing[CompositeSearchEventSource]].thenReturn(new CompositeSearchEventSource(List[String](EventFixture.DIALCODE_METRIC_CREATE)))
    when(mockKafkaUtil.kafkaStringSink(jobConfig.kafkaErrorTopic)).asInstanceOf[OngoingStubbing[CompositeSearchFailedEventSink]].thenReturn(new CompositeSearchFailedEventSink)
    new SearchIndexerStreamTask(jobConfig, mockKafkaUtil).process()
  ...
  "Composite Search Indexer" should " update the Dialcode Metrics Data " in {
    when(mockKafkaUtil.kafkaJobRequestSource[Event](jobConfig.kafkaInputTopic)).asInstanceOf[OngoingStubbing[CompositeSearchEventSource]].thenReturn(new CompositeSearchEventSource(List[String](EventFixture.DIALCODE_METRIC_UPDATE)))
    when(mockKafkaUtil.kafkaStringSink(jobConfig.kafkaErrorTopic)).asInstanceOf[OngoingStubbing[CompositeSearchFailedEventSink]].thenReturn(new CompositeSearchFailedEventSink)
    new SearchIndexerStreamTask(jobConfig, mockKafkaUtil).process()
  ...
  "Composite Search Indexer" should " create and delete the Dialcode Metrics Data " in {
    embeddedElastic.deleteIndices()
    when(mockKafkaUtil.kafkaJobRequestSource[Event](jobConfig.kafkaInputTopic)).asInstanceOf[OngoingStubbing[CompositeSearchEventSource]].thenReturn(new CompositeSearchEventSource(List[String](EventFixture.DIALCODE_METRIC_CREATE, EventFixture.DIALCODE_METRIC_DELETE)))
    when(mockKafkaUtil.kafkaStringSink(jobConfig.kafkaErrorTopic)).asInstanceOf[OngoingStubbing[CompositeSearchFailedEventSink]].thenReturn(new CompositeSearchFailedEventSink)
    new SearchIndexerStreamTask(jobConfig, mockKafkaUtil).process()
  ...
  "Composite Search Indexer" should " do nothing for the Dialcode Metrics Data due to UNKNOWN Operation " in {
    when(mockKafkaUtil.kafkaJobRequestSource[Event](jobConfig.kafkaInputTopic)).asInstanceOf[OngoingStubbing[CompositeSearchEventSource]].thenReturn(new CompositeSearchEventSource(List[String](EventFixture.DIALCODE_METRIC_UNKNOWN)))
    when(mockKafkaUtil.kafkaStringSink(jobConfig.kafkaErrorTopic)).asInstanceOf[OngoingStubbing[CompositeSearchFailedEventSink]].thenReturn(new CompositeSearchFailedEventSink)
    new SearchIndexerStreamTask(jobConfig, mockKafkaUtil).process()
    val elasticUtil = new ElasticSearchUtil(jobConfig.esConnectionInfo, jobConfig.dialcodeMetricIndex, jobConfig.dialcodeMetricIndexType)
    val data = elasticUtil.getDocumentAsString("QR1234")
  ...
  "Composite Search Indexer" should " do nothing due to UNKNOWN Node Type " in {
    when(mockKafkaUtil.kafkaJobRequestSource[Event](jobConfig.kafkaInputTopic)).asInstanceOf[OngoingStubbing[CompositeSearchEventSource]].thenReturn(new CompositeSearchEventSource(List[String](EventFixture.UNKNOWN_NODE_TYPE)))
    when(mockKafkaUtil.kafkaStringSink(jobConfig.kafkaErrorTopic)).asInstanceOf[OngoingStubbing[CompositeSearchFailedEventSink]].thenReturn(new CompositeSearchFailedEventSink)
    new SearchIndexerStreamTask(jobConfig, mockKafkaUtil).process()
    BaseMetricsReporter.gaugeMetrics(s"${jobConfig.jobName}.${jobConfig.totalEventsCount}").getValue() should be(1)
  ...
  "Composite Search Indexer" should " do nothing due to FALSE value of INDEX of the Data " in {
    when(mockKafkaUtil.kafkaJobRequestSource[Event](jobConfig.kafkaInputTopic)).asInstanceOf[OngoingStubbing[CompositeSearchEventSource]].thenReturn(new CompositeSearchEventSource(List[String](EventFixture.INDEX_FALSE)))
    when(mockKafkaUtil.kafkaStringSink(jobConfig.kafkaErrorTopic)).asInstanceOf[OngoingStubbing[CompositeSearchFailedEventSink]].thenReturn(new CompositeSearchFailedEventSink)
    new SearchIndexerStreamTask(jobConfig, mockKafkaUtil).process()
    BaseMetricsReporter.gaugeMetrics(s"${jobConfig.jobName}.${jobConfig.totalEventsCount}").getValue() should be(1)
    BaseMetricsReporter.gaugeMetrics(s"${jobConfig.jobName}.${jobConfig.skippedEventCount}").getValue() should be(1)
  ...
  "Composite Search Indexer" should " give error for the External Dialcode Data due to UNKNOWN objectType " in {
    embeddedElastic.deleteIndices()
    when(mockKafkaUtil.kafkaJobRequestSource[Event](jobConfig.kafkaInputTopic)).asInstanceOf[OngoingStubbing[CompositeSearchEventSource]].thenReturn(new CompositeSearchEventSource(List[String](EventFixture.DATA_NODE_FAILED)))
    when(mockKafkaUtil.kafkaStringSink(jobConfig.kafkaErrorTopic)).asInstanceOf[OngoingStubbing[CompositeSearchFailedEventSink]].thenReturn(new CompositeSearchFailedEventSink)
    intercept[Exception] {
      new SearchIndexerStreamTask(jobConfig, mockKafkaUtil).process()
    }
  ...
  "Search Indexer" should " ignore the event with restricted ObjectTypes " in {
    when(mockKafkaUtil.kafkaJobRequestSource[Event](jobConfig.kafkaInputTopic)).asInstanceOf[OngoingStubbing[CompositeSearchEventSource]].thenReturn(new CompositeSearchEventSource(List[String](EventFixture.DATA_NODE_IGNORE)))
    new SearchIndexerStreamTask(jobConfig, mockKafkaUtil).process()
